...
class WorkerManagerService:
    ...
    async def run(self) -> None:
        """
        This function will be called once to start the worker manager service.
        Returns:
            None
        """
        self.info_logger.log("Starting WorkerManagerService...", log_type=LogType.INFO)
        self._is_running = True
        # ironically this is a periodic worker - can it queue itself...
        while True:
            last_handled_worker_item = None
            while (worker_item := await self._worker_queue.get()).next_run <= discord.utils.utcnow().timestamp():
    ...
    async def add_worker(self, worker_callback: callable,
                         worker_name: str,
                         next_run: datetime = discord.utils.utcnow(),
                         **kwargs) -> None:
        """
        This function will be called to add a worker to the queue.
        Args:
            worker_callback (callable): Reference to the worker function
            worker_name (str): Name of the worker
            next_run (datetime): Timestamp of when the worker should run next
    ...
    async def start_worker(self, worker_item: 'WorkerItem') -> None:
        """
        This function will be called to start a worker.
        Args:
            worker_item (WorkerItem): WorkerItem object
        Returns:
            None
        """
        try:
            ...
            wait_time = WORKER_RETRY_ON_ERROR_DELAY
        ...
        await self.add_worker(worker_callback=worker_item.worker_callback,
                              ...
                              next_run=(discord.utils.utcnow().timestamp() + wait_time) if wait_time else None,
                              **worker_item.kwargs)
...
class WorkerItem:
    def __init__(self, next_run: datetime, worker_callback: callable, worker_name: str, **kwargs) -> None:
        """
        This class will be used to represent a worker in the worker queue.
        Its primary purpose is to allow for comparison.
        Args:
            next_run (datetime): Timestamp of when the worker should run next
            worker_callback (callable): Reference to the worker function
            worker_name (str): Name of the worker
